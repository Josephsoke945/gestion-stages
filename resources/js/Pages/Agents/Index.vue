...
const form = useForm({
  ...
  date_embauche: '',
  role_agent: '',
  structure_selectionnee_id: null, // Ajout pour gérer la structure
});
...
watch(() => form.role_agent, async (newRole) => {
  if (newRole === 'RS') {
    try {
      ...
      console.log('Structures disponibles:', response.data);
    ...
    form.structure_selectionnee_id = null;
  }
});
...
function openModal(agent = null) {
  ...
  if (agent) {
    ...
    form.structure_selectionnee_id = agent.structure_id || null;
            ...
            <form @submit.prevent="submit" class="px-6 py-4">
              ...
              <div v-if="step === 2" class="space-y-4">
                ...
                <div v-if="form.role_agent === 'RS'">
                  ...
                  <select v-model="form.structure_selectionnee_id"
                    class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    required>
